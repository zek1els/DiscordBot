...
- **Login:** Everyone uses **Login with Discord**. In [Developer Portal](https://discord.com/developers/applications) → your app → OAuth2 → Redirects, add `https://YOUR_RAILWAY_DOMAIN/api/auth/discord/callback`. **Client ID** = Application ID on the app page; **Client Secret** = OAuth2 → Client Secret (click Reset to reveal).
- **Admins:** Set `ADMIN_DISCORD_IDS` to your Discord user ID (enable Developer Mode in Discord, right-click your name → Copy User ID). Admins see all scheduled messages; others see only their own.
...
3. Open that URL in your browser. Click **Login with Discord** to use the scheduler. Set `ADMIN_DISCORD_IDS` to your user ID to see all schedules as an admin.
...
- **Web app:** Served at your Railway URL; log in with Discord. Admins (in `ADMIN_DISCORD_IDS`) see all scheduled messages; others see only their own.
